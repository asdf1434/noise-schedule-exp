...
#SBATCH --array=0-119
#SBATCH --output=logs/slurm/slurm_mnist_lowres_%A_%a.out
...
echo "========================================"
echo "Array task $IDX: dataset=mnist conditioning=lowres dist=$DIST_NAME seed=$SEED"
echo "========================================"

case "$DIST_NAME" in
  uniform)
    python -u train.py --dataset mnist --conditioning lowres --train_dist uniform --seed "$SEED"
    ;;
  logit_normal)
    python -u train.py \
        --dataset mnist \
...
echo "Once ALL 120 array tasks finish, re-run the existing eval chain for mnist (run_exp1_eval*.sh -- it defaults to --dataset mnist)"
echo "(run_exp1_eval_array.sh -> run_exp1_eval_merge.sh) to pick up these"
