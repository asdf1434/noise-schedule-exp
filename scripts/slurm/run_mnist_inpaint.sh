...
# ==========================================
# Q2 conditioning sweep: mnist with --conditioning inpaint, the same 6
...
#SBATCH --array=0-119
#SBATCH --output=logs/slurm/slurm_mnist_inpaint_%A_%a.out
...
echo "========================================"
echo "Array task $IDX: dataset=mnist conditioning=inpaint dist=$DIST_NAME seed=$SEED"
echo "========================================"

case "$DIST_NAME" in
  uniform)
    python -u train.py --dataset mnist --conditioning inpaint --train_dist uniform --seed "$SEED"
    ;;
  logit_normal)
    python -u train.py \
        --dataset mnist \
