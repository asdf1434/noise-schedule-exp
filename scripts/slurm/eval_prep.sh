...
# ==========================================
# Stage 1 of the FID chain: make sure everything the scoring stage reads
# already exists, for one experiment's datasets.
#
# Three things, in order:
#   1. reference images   (only for datasets listed in GENERATE_REAL)
#   2. cached Inception stats for each dataset  (cache_real_stats.py)
#   3. seeded per-shard result files            (seed_fid_shards.py)
#
# Step 3 is the one that is easy to skip and expensive to get wrong. The merge
# stage rebuilds results/master_fid_results.json from the shard files alone, so
# any cell that no shard happens to rescore is dropped. Seeding the shards from
# the current master first makes the merge additive. Older hand-written chains
# only did this for some experiments; here it always runs.
#
# Submitted by submit.sh; not usually run by hand.
# ==========================================
#SBATCH --job-name=eval_prep
...
#SBATCH --output=logs/slurm/slurm_eval_prep_%j.out
...
echo "========================================"
echo "$EXPERIMENT eval prep: datasets ${EVAL_DATASETS[*]}, $EVAL_SHARDS shards"
echo "========================================"
...
for DS in "${GENERATE_REAL[@]}"; do
    run python -u -m src.generate_real_samples --dataset "$DS"
done

for DS in "${EVAL_DATASETS[@]}"; do
    run python -u cache_real_stats.py --dataset "$DS"
done

# Nothing to preserve before the first scoring run, so a missing master is fine.
if [ -f results/master_fid_results.json ]; then
    for DS in "${EVAL_DATASETS[@]}"; do
        run python -u scripts/monitor/seed_fid_shards.py --dataset "$DS" --num_shards "$EVAL_SHARDS"
    done
else
    echo "No results/master_fid_results.json yet -- nothing to seed, skipping."
fi

echo "$EXPERIMENT eval prep complete."
