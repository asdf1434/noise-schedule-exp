...
# ==========================================
# Q2 conditioning sweep: cifar10 with --conditioning lowres, the same 6
# training distributions x 20 seeds used for this dataset's cond-none sweep.
# CIFAR-10 trains at native 32x32 (converted to grayscale), same time budget as run_cifar10.sh.
#
...
#SBATCH --array=0-119
#SBATCH --output=logs/slurm/slurm_cifar10_lowres_%A_%a.out
...
echo "========================================"
echo "Array task $IDX: dataset=cifar10 conditioning=lowres dist=$DIST_NAME seed=$SEED"
echo "========================================"

case "$DIST_NAME" in
  uniform)
    python -u train.py --dataset cifar10 --conditioning lowres --train_dist uniform --seed "$SEED"
    ;;
  logit_normal)
    python -u train.py \
        --dataset cifar10 \
...
echo "Once ALL 120 array tasks finish, re-run the existing eval chain for cifar10"
echo "(run_cifar10_eval_array.sh -> run_cifar10_eval_merge.sh) to pick up these"
