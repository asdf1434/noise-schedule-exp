...
# ==========================================
# Q2 conditioning sweep: fashion_mnist with --conditioning inpaint, the same 6
# training distributions x 20 seeds used for this dataset's cond-none sweep.
# Fashion-MNIST is 28x28, same time budget as run_fashionmnist.sh.
#
...
# the existing run_fashion_mnist_eval*.sh chain already scores every
# experiment under this dataset regardless of conditioning (evaluate_fid.py
# filters by dataset only). Just re-run:
#   sbatch scripts/slurm/run_fashion_mnist_eval_array.sh   (after stats are cached)
#   sbatch --dependency=afterok:<ARRAY_JOBID> scripts/slurm/run_fashion_mnist_eval_merge.sh
# ==========================================
#SBATCH --job-name=fashionmnist_inpaint
...
#SBATCH --array=0-119
#SBATCH --output=logs/slurm/slurm_fashionmnist_inpaint_%A_%a.out
...
echo "========================================"
echo "Array task $IDX: dataset=fashion_mnist conditioning=inpaint dist=$DIST_NAME seed=$SEED"
echo "========================================"

case "$DIST_NAME" in
  uniform)
    python -u train.py --dataset fashion_mnist --conditioning inpaint --train_dist uniform --seed "$SEED"
    ;;
  logit_normal)
    python -u train.py \
...
echo "Once ALL 120 array tasks finish, re-run the existing eval chain for fashion_mnist"
echo "(run_fashion_mnist_eval_array.sh -> run_fashion_mnist_eval_merge.sh) to pick up these"
