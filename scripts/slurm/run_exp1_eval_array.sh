...
# array (8 independent tasks, one process each, each on its own GPU), the
...
# one shared log.
#
# Shard count: evaluate_fid.py now runs on GPU with a resident InceptionV3
# feature extractor (built once per process, not once per folder), so
# per-folder cost should be seconds rather than the ~3 min/folder CPU
# baseline this used to be sized for. 8 shards is a starting point to
# re-tune once real GPU throughput is measured -- with that few tasks, most
# of the previous 64-shard split was Slurm queue/startup overhead rather
