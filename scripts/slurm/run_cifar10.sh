...
# ==========================================
# CIFAR-10 counterpart to run_eurosat.sh: the same 6 training distributions
# x 20 seeds, at CIFAR-10's native 32x32 resolution (--dataset cifar10).
# CIFAR-10 is natively RGB; src/datasets.py's get_cifar10_dataloaders
# collapses it to grayscale (same luminance treatment as EuroSAT) so it fits
# the model/conditioning/FID pipeline's single-channel assumption.
#
# Before submitting this for the first time, the cifar10 real-image
...
#   python cache_real_stats.py --dataset cifar10   # (run_cifar10_eval.sh does this)
#
...
# ==========================================
#SBATCH --job-name=cifar10_train_dists
...
#SBATCH --array=0-119
#SBATCH --output=logs/slurm/slurm_cifar10_train_dists_%A_%a.out
...
echo "========================================"
echo "Array task $IDX: dataset=cifar10 dist=$DIST_NAME seed=$SEED"
echo "========================================"

case "$DIST_NAME" in
  uniform)
    python -u train.py --dataset cifar10 --train_dist uniform --seed "$SEED"
    ;;
  logit_normal)
    python -u train.py \
        --dataset cifar10 \
...
echo "Once ALL 120 array tasks finish, run evaluate_fid.py --dataset cifar10 separately -- see"
echo "scripts/slurm/run_cifar10_eval.sh (submit with --dependency=afterok:<this array job's ID>)."
echo "========================================"
